...
class GDumb(BaseStrategy):
    ...
    def __init__(self, model: Module, optimizer: Optimizer, criterion,
                 ...
                 eval_mb_size: int = None, device=None,
                 plugins: Optional[List[StrategyPlugin]] = None,
                 evaluator: EvaluationPlugin = default_logger, eval_every=-1,reset=True,buffer='class_balance',
                 alpha_mode=None,alpha_value=None):
        ...
        gdumb = GDumbPlugin(mem_size,reset=reset,buffer=buffer,alpha_mode=alpha_mode,alpha_value=alpha_value)
        if plugins is None:
            plugins = [gdumb]
        else:
